...
fn esp(
    ...
    //     draw_list,
    //     ui,
    //     window_size.map(|v| v as f32).map(|v| v / 2.0),
    //     players,
    //     &unreal_radar::RadarConfig {
    //         size: 100.0,
    //         background_color: 0,
    //         line_width: 2.0,
    //         scale: 200.0,
    ...
    //         direction_color: 1,
    //         indicator_scale: 1.5,
    //         indicator_shape: unreal_radar::RadarShape::Arrow,
...
pub fn players(ctx: &ThreadCtx<AppData>) -> ThreadFlow {
    ...
    for i in 0..20 {
        ...
        if let Ok(abs_rotation) = read::<Vector3>(scene_node + CGameSceneNode::m_angAbsRotation) {
            ...
            read::<Vector3>(scene_node + CGameSceneNode::m_angRotation)
        {
            player.view_yaw = local_rotation.y;
        }
        ...
        if player.is_local {
            let abilities_comp = pawn + C_CitadelPlayerPawn::m_CCitadelAbilityComponent;
            let vec_base = abilities_comp + CCitadelAbilityComponent::m_vecAbilities;

            let abilities_ptr = match read::<usize>(vec_base + 0x8) {
                Ok(ptr) if ptr != 0 => ptr,
                _ => {
                    players.push(player);
                    continue;
            ...
            // Network vector count; clamp to avoid out-of-bounds handle reads.
            let ability_count = match read::<i32>(vec_base + 0x10) {
                Ok(count) => count.clamp(0, 24) as usize,
                Err(_) => 24,
            };

            player.abilities.reserve(ability_count);
            for ability_idx in 0..ability_count {
                let handle = match read::<u32>(abilities_ptr + ability_idx * std::mem::size_of::<u32>())
                {
                    Ok(handle) if handle != 0 => handle as usize,
                    _ => continue,
                };

                let handle_index = handle & ENTITY_INDEX_MASK;
                let page_ptr_addr =
                    entity_list + ENTITY_LIST_PAGE_OFFSET + ((handle_index >> 9) * std::mem::size_of::<usize>());
                let ent_entry = match read::<usize>(page_ptr_addr) {
                    Ok(entry) if entry != 0 => entry,
                    _ => continue,
                };

                let ability_entity = match read::<usize>(ent_entry + ENTITY_ENTRY_STRIDE * (handle_index & 0x1ff))
                {
                    Ok(entity) if entity != 0 => entity,
                    _ => continue,
                };
                ...
                let channeling = match read::<bool>(ability_entity + C_CitadelBaseAbility::m_bChanneling) {
                    Ok(val) => val,
                    Err(_) => continue,
                };

                let cooldown_start = match read::<f32>(ability_entity + C_CitadelBaseAbility::m_flCooldownStart) {
                    Ok(val) => val,
                    Err(_) => continue,
                };

                let cooldown_end = match read::<f32>(ability_entity + C_CitadelBaseAbility::m_flCooldownEnd) {
                    Ok(val) => val,
                    Err(_) => continue,
                };

                let data_ptr = read::<usize>(ability_entity + 0x390).unwrap_or(0);
    ...
    for cached in cached_entities {
        ...
        entity.attackable = match entity.e_type {
            EntityType::Soul => {
                ...
                let cur_time = skip_err!(read::<f32>(
                    cached.entity_ptr + C_BaseEntity::m_flSimulationTime
                ));
